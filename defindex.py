...
class Kti:
    ...
    def __init__(self, n_30=constant.N_30, *i):
        self.kti = i   # 如果*i没有赋值，那么为（）的空tuple
        self.n_30 = n_30
        if len(self.kti) == 2:
            self.kti += (constant.N_5 - 1,)
        elif len(self.kti) == 1:
            self.kti += (n_30 - 1, constant.N_5 - 1)
        elif len(self.kti) == 0:
    ...
    def __add__(self, other):
        ...
        kti = Kti(self.n_30, *self.inv_value(v, self.n_30))
        return kti
    ...
    def value(self, *kti):
        ...
        # else:
        #     kti = kti[0]
        value = kti[0] * self.n_30 * constant.N_5 + kti[1] * constant.N_5 + kti[2] + 1
        return value

    @staticmethod
    def inv_value(v, n_30):
        """
        ...
        kti = (floor((v - 1) / n_30 / constant.N_5),)
        kti += (floor(((v - 1) % (n_30 * constant.N_5)) / constant.N_5), )
        kti += (((v - 1) % (n_30 * constant.N_5)) % constant.N_5,)
        return kti
...
def get_kti_seq(seq, init_p, n_30, value):
    ...
    if not seq:
        ...
        o = Kti(n_30, *init_p)
        o_v = o.value()
        seq_v = list(map(lambda x: o_v + x * value, seq))
        seq_kti = list(map(Kti.inv_value, seq_v, [constant.N_30]*len(seq_v)))
...
class Kn:
    ...
    @staticmethod
    def v_to_kn(v, lev, n_30=constant.N_30):
        from math import floor
        den = [n_30 * constant.N_5, constant.N_5, 1]
